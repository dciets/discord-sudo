...
class SoundboardAdd extends Command {
  ...
  public async execute(message: DiscordJS.Message, ...args: string[]) {
    ...
    if (args[1] === "me") {
      ...
      if (!ffmpeg.stdout || !ffmpeg.stdin) throw new Error("missing stds");
      ...
      if ((await waitFor(ffmpeg, "exit")) !== 0) return message.react("❌");
      ...
      const recorders: Promise<string>[] = message.member.voice.channel.members.map(
        (m) => {
      ...
      const ids: string[] = (await Promise.all(recorders)).filter((s) => !!s);
