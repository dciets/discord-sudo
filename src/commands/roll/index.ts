...
class Roll extends Command {
  ...
  public async execute(message: DiscordJS.Message, ...args: string[]) {
    if (args.length === 0)
      return message.reply(__mf("rolled a %s", random.range(1, 6).toString()));
    else if (args.length === 1) {
      if (/^\d+$/.test(args[0]))
        return message.reply(
          __mf("rolled a %s", random.range(1, Number(args[0])).toString())
        );
      ...
      const rolls = (
        args.map((arg) => {
          if (/^\d+$/.test(arg)) return [random.range(1, Number(args[0]))];
      ...
      const total = rolls.reduce((a, b) => a + b.reduce((c, d) => c + d, 0), 0);
